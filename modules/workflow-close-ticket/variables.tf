...
variable "lambda_ticket_comments" {
  description = "ARN of the PicoFun Lambda function used to fetch the ticket comments"
  type        = string
}

variable "lambda_ticket_update" {
  description = "ARN of the PicoFun Lambda function used to update the ticket. Override to use the dummy function to run workflows in report only mode."
  type        = string
}

variable "lambda_user_get" {
  description = "ARN of the PicoFun Lambda function used to fetch user details"
  type        = string
}
...
locals {
  lambda_ticket_comments_name = split(":", provider::aws::arn_parse(var.lambda_ticket_comments).resource)[1]
  lambda_ticket_update_name   = split(":", provider::aws::arn_parse(var.lambda_ticket_update).resource)[1]
  lambda_user_get_name        = split(":", provider::aws::arn_parse(var.lambda_user_get).resource)[1]

  eb_role_name  = join("", [(var.role_namespace != "" ? "${var.role_namespace}-" : ""), "${var.application_name}-eb-solved-ticket"])
  sfn_role_name = join("", [(var.role_namespace != "" ? "${var.role_namespace}-" : ""), "${var.application_name}-sfn-solved-ticket"])

  sfn_name = "${var.application_name}-solved-ticket"
